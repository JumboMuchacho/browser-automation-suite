...
import argparse
import sys
import shlex  # Added for secure command splitting
...
def run_cmd(cmd: str) -> str:
    try:
        # SECURE FIX: Convert string to list and disable shell execution
        # This prevents command injection and satisfies the security scanner.
        args = shlex.split(cmd)
        out = subprocess.check_output(
            args,
            shell=False,  # Now secure
            text=True,
...
def get_cpu_id() -> str:
    if platform.system() == "Windows":
        # Passing the command string to run_cmd which now handles it safely
